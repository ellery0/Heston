...
from nn import kolmogorov_train_and_test
...
kappa, sigma = 0.6, 0.2
theta, rho = 0.04, -0.8
S_0 = tf.random_uniform((batch_size, int(d/2)),minval=90., maxval=110., dtype=dtype)
V_0 = tf.random_uniform((batch_size,int(d/2)),minval=0.02, maxval = 0.2,dtype=dtype)
h=T/N
neurons=[d+50,d+50,1]
train_steps = 750000
...
xi = tf.reshape(tf.stack([S_0, V_0],axis=2),(batch_size, d))


def phi(x,axis=1):
    return np.exp(-alpha * T) \
        * tf.maximum(K - tf.reduce_mean(tf.exp(x), axis=axis,keepdims=True), 0.)


def sde_body(idx, s, v, samples):
    _sqrt_v = tf.sqrt(v)
    dw_1 =  tf.random_normal(shape=(samples, batch_size, int(d / 2)),
                 stddev=np.sqrt(h), dtype=dtype)
    dw_2 =  rho * dw_1 + np.sqrt(1. - rho ** 2) \
    * tf.random_normal(shape=(samples, batch_size, int(d / 2)),
                       stddev=np.sqrt(h), dtype=dtype)
    ...
    return tf.add(idx,1),s+(alpha-v/2.)*h+_sqrt_v*dw_1,\
            tf.maximum(tf.maximum(np.float32(sigma / 2. * np.sqrt(h)),
                      tf.maximum(np.float32(sigma / 2. * np.sqrt(h)),
                                 _sqrt_v)
                      + sigma / 2. * dw_2) ** 2  + (kappa * theta - sigma**2/4.-kappa*v)*h,0.)
...
loop_var_mc = (tf.constant(0), tf.ones((mc_samples_ref, batch_size, int(d / 2)),
                                   dtype) * tf.log(S_0),
           tf.ones((mc_samples_ref, batch_size, int(d / 2)),
                   dtype) * V_0)
loop_var = (tf.constant(0), tf.ones((1, batch_size, int(d / 2)),dtype) * tf.log(S_0),
            tf.ones((1, batch_size, int(d / 2)), dtype) * V_0)
